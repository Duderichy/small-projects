...
def group_mean(vals, group):
    ...
    for val, group_id in zip(vals, group):
        if group_id not in lookup and val:
            lookup[group_id] = [0, 0]
        if val:
            total_counter = lookup[group_id]
            total_counter[0] += val
            total_counter[1] += 1
    # calculates average from count and total in lookup table
    lookup_means = {group_id : total_counter[0] / total_counter[1]
                    for group_id, total_counter in lookup.items()}
    # returns list of means for each group id in proper order
    return [lookup_means[group_id] for group_id in group]

def groups_means(vals, groups):
    """
    Calculates the means for a list of groups
    """
    return [group_mean(vals, group) for group in groups]

def calculate_weighted_avg(group_mean_lists, weights):
    """
    Calculates the weighted averages from a list of group mean lists,
    and weights
    """
    weighted_vals = [[weights[i] * val for val in group_mean]
                     for i, group_mean in enumerate(group_mean_lists)]
    new_list = [0 for _ in range(len(weighted_vals[0]))]
    for group_weighted in weighted_vals:
        for i, val in enumerate(group_weighted):
            new_list[i] += val
    return new_list
...
def group_adjust(vals, groups, weights):
    ...
    if not all(x == len(vals) for x in map(len, groups + [vals])):
        raise ValueError
    weighted_averages = calculate_weighted_avg(groups_means(vals, groups),
                                                weights)
    return [val - weighted_average if val else None
            for val, weighted_average in zip(vals, weighted_averages)]
